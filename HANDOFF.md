...
> **שלב נוכחי:** חלק 2 בוצע — תשתית חשבונות (2A) + פורטל מבוגר↔ילד (2B). 308 tests, precache 163.
> **הבא בתור:** חלק 3 (תמונות אישיות + פרטיות) · deploy Cloud Functions (`firebase deploy --only functions` מ-`functions/`) · הגדרת admin ראשוני ב-Firebase console. היסטוריה מלאה: `docs/CHANGELOG.md`.
