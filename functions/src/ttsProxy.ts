...
export const ttsProxy = onCall(
  ...
  async (request) => {
    ...
    // מכסה יומית פר-uid — הגנת-תקציב מפני חשבון מאושר שנפרץ. מכוונן ל-~10 ילדים (פר-uid).
    await enforceRateLimit(request.auth.uid, 'tts:daily', { windowMs: 86_400_000, max: 3_000 });
